...
void test(int &lstart, ImageFolderReader* &reader,ImageFolderReader* &reader_right, int &lend,int view_num_index) {
...
int main( int argc, char** argv )
{
    ...
    if(1)
    {
        ...
        int count = 0;
        std::string line;
        while (std::getline(savedFile0, line)) {
            ++count;
        }
        finalFile_0 << std::string("# .PCD v.6 - Point Cloud Data file format\n");
        finalFile_0 << std::string("FIELDS x y z\n");
        finalFile_0 << std::string("SIZE 4 4 4\n");
        finalFile_0 << std::string("TYPE F F F\n");
        finalFile_0 << std::string("COUNT 1 1 1\n");
        finalFile_0 << std::string("WIDTH ") << count<< std::string("\n");
        finalFile_0 << std::string("HEIGHT 1\n");
        finalFile_0 << std::string("#VIEWPOINT 0 0 0 1 0 0 0\n");
        finalFile_0 << std::string("POINTS ") << count << std::string("\n");
        finalFile_0 << std::string("DATA ascii\n");


        std::ifstream savedFile00("0.pcd");
        while (!savedFile00.eof())
        {
            finalFile_0.put(savedFile00.get());
        }

        std::ofstream finalFile_1("final_1.pcd");
        std::ifstream savedFile1("1.pcd");
        count = 0;
        while (std::getline(savedFile1, line)) {
            ++count;
        }

        finalFile_1 << std::string("# .PCD v.6 - Point Cloud Data file format\n");
        finalFile_1 << std::string("FIELDS x y z\n");
        finalFile_1 << std::string("SIZE 4 4 4\n");
        finalFile_1 << std::string("TYPE F F F\n");
        finalFile_1 << std::string("COUNT 1 1 1\n");
        finalFile_1 << std::string("WIDTH ") << count << std::string("\n");
        finalFile_1 << std::string("HEIGHT 1\n");
        finalFile_1 << std::string("#VIEWPOINT 0 0 0 1 0 0 0\n");
        finalFile_1 << std::string("POINTS ") << count << std::string("\n");
        finalFile_1 << std::string("DATA ascii\n");


        std::ifstream savedFile11("1.pcd");
        while (!savedFile11.eof())
        {
            finalFile_1.put(savedFile11.get());
        }
    ...
    ImageFolderReader* reader2 = new ImageFolderReader(source+"/image_0", calib, gammaCalib, vignette);
    ImageFolderReader* reader2_right = new ImageFolderReader(source+"/image_1", calib, gammaCalib, vignette);

	reader->setGlobalCalibration();
	reader_right->setGlobalCalibration();

    reader2->setGlobalCalibration();
    reader2_right->setGlobalCalibration();
    ...
    int lstart2 = lstart; int lend2 = lend;
    std::thread runthread2([&]() {
        test(lstart, reader,reader_right, lend,0);
    });
    std::thread runthread3([&]() {
        test(lstart2, reader2, reader2_right, lend2,1);
    });
