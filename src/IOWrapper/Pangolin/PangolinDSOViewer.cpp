...
PangolinDSOViewer::PangolinDSOViewer(class pclSetting* &pclSetting_arg,int w, int h, bool startRunThread)
{
...
void PangolinDSOViewer::run()
{
    ...
    // Add by Yohan.
    //thread_local pangolin::Var<bool> settings_savePCButton("ui.savePointCloud",false,false);
    pangolin::Var<bool> settings_savePCButton("ui.save-"+boost::to_string(_pclSetting->view_num_index),false,false);
	...
	while( !pangolin::ShouldQuit() && running )
	{
        ...
        if(settings_savePCButton.Get())
        {
            ...
            while (1)
            {
                if (!_pclSetting->isWritePCL)
                {
                    _pclSetting->isSavePCL = false;
                    break;
                }
            ...
            while (1)
            {
                ...
                {
                    //pclSetting->strSaveFileName = boost::to_string(boost::this_thread::get_id())+".pcd";
                    //pclSetting->strTmpFileName = boost::to_string(boost::this_thread::get_id())+"_tmp.pcd.pango";
                    std::ofstream finalFile(_pclSetting->strSaveFileName);
                    ...
                    finalFile << std::string("WIDTH ") << _pclSetting->numPCL << std::string("\n");
                    finalFile << std::string("HEIGHT 1\n");
                    finalFile << std::string("#VIEWPOINT 0 0 0 1 0 0 0\n");
                    finalFile << std::string("POINTS ") << _pclSetting->numPCL << std::string("\n");
                    finalFile << std::string("DATA ascii\n");

                    std::ifstream savedFile(_pclSetting->strTmpFileName);
                    ...
                    //printf("PCL File for '%s' is saved.\n", _pclSetting->strSaveFileName.c_str());
