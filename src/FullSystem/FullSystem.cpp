...
FullSystem::FullSystem(class pclSetting* &pclSetting_arg)
{
...
//        ////add by pyl
        ...
        if (_pclSetting->view_num_index != 0)
        {
            while(v0_key_frame_list.empty() || fh->shell->incoming_id > v0_key_frame_list.back())
            {
            ...
            {
                if(fh->shell->incoming_id == f_id){
                    std::cout<<"add_flag:"<<f_id<<std::endl;
                    add_flag =1;
                    break;
                }
            }
...
void FullSystem::mappingLoop()
{
	...
	while(runMapping)
	{
        ...
        std::cout<<"do something mapping"<<std::endl;
...
void FullSystem::makeKeyFrame( FrameHessian* fh, FrameHessian* fh_right)
{
    ////add by pyl
    if(_pclSetting->view_num_index==0){
        v0_key_frame_list.push_back(fh->shell->incoming_id);
    }
	...
	if(allKeyFramesHistory.size() <= 4 && _pclSetting->view_num_index==0)
	{
		if(allKeyFramesHistory.size()==2 && rmse > 20*benchmark_initializerSlackFactor)
		{
			printf("I THINK INITIALIZATINO FAILED! Resetting.\n");
			initFailed=true;
            v0_key_frame_list.clear();
		}
